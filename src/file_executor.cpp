...
void affinity_migration(const MappedTensor& input, MappedTensor& output, const EffectSpec& effect, const LocalProgress& progress) {
    const auto& shape = input.shape();
    const auto cell = 1 + static_cast<std::size_t>(std::round(
        std::clamp(effect.values[0], 0.0F, 1.0F) * static_cast<float>(std::max(shape.w, shape.h) - 1)));
    const auto agreement = std::clamp(effect.values[1], 0.0F, 1.0F);
    const auto rounds = std::clamp<std::size_t>(static_cast<std::size_t>(std::round(effect.values[2])), 1, 12);
    const auto motion_response = std::clamp(effect.values[3], 0.0F, 1.0F);
    require_option(effect.options[0], 6, "affinity palette classes");
    const auto classes = effect.options[0] + 2;
    const auto rows = (shape.h + cell - 1) / cell;
    const auto columns = (shape.w + cell - 1) / cell;
    ...
    for (std::size_t t = 0; t < shape.t; ++t) {
        ...
        for (std::size_t row = 0; row < rows; ++row) for (std::size_t column = 0; column < columns; ++column) {
            ...
            const auto y = std::min(shape.h - 1, row * cell);
            const auto x = std::min(shape.w - 1, column * cell);
        ...
        for (std::size_t row = 0; row < rows; ++row) for (std::size_t column = 0; column < columns; ++column) {
            const auto index = index_of(row, column);
            const auto y0 = row * cell;
            const auto x0 = column * cell;
            ...
            remembered_offset_y[index] += (static_cast<float>(origin_row * cell) - static_cast<float>(y0) - remembered_offset_y[index]) * response;
            remembered_offset_x[index] += (static_cast<float>(origin_column * cell) - static_cast<float>(x0) - remembered_offset_x[index]) * response;
            for (std::size_t y = y0; y < std::min(shape.h, y0 + cell); ++y) for (std::size_t x = x0; x < std::min(shape.w, x0 + cell); ++x) {
                for (std::size_t c = 0; c < shape.c; ++c) {
                    output.mutable_data()[linear(t, y, x, c, shape)] = sample_mapped(input, static_cast<float>(t),
...
void apply(
    ...
    switch (effect.kind) {
        ...
        case EffectOperation::BitplaneForge:
            ...
            return;
        case EffectOperation::AffinityMigration:
            affinity_migration(input, output, effect, progress);
            return;
