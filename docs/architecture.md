...
`SessionStore` separates lightweight `CreativeSessionState` from decoded media, render tasks, playback and queue state. `UndoManager` keeps at most 100 creative operations; slider drags are coalesced into one operation. Preview always updates automatically after 450 ms of inactivity, or 800 ms for global-cost effects, and owns cancellation only for its debounce and proxy render. A full export retains an independent immutable stack snapshot.
...
3. **Polar Time Warp** jointly warps fractional time, radius and polar angle into animated braids. Its effect coordinate system can rotate independently of the canvas; Time Loom can replace the non-periodic angular ramp with a circular field to remove the `atan2` branch-cut seam. Kaleido Fold creates moving spatial sectors; Event Horizon bends radius into orbiting temporal echoes.
...
10. **Axis Datamosh** carries samples along Time, X or Y after deterministic edge, luma or random triggers; luma can trigger from either the bright or dark side of its threshold.
11. **Seamless Loop** overlaps the tail with the beginning, optionally staggers the transition per pixel by luma or minimum difference, interpolates spatial FFT magnitude/phase for Spectral Morph, or emits a forward/reverse ping-pong tensor. Spectral Morph exposes its FFT contribution, high-frequency attenuation and phase timing. Overlap modes can place the generated transition at the start or end of the resulting timeline while preserving the same circular boundary, and shorten `T`; Ping-Pong emits `2T-2` frames.
...
13. **Sync Loss** assigns coherent row or column bands a time-drifting identity and shifts them on the orthogonal axis. Band size is normalized from one pixel to the full frame; deterministic noise, luma or edges gate the tear.
...
16. **Block Address Corruption** hashes the seed, hold epoch and destination block into a stable mapping and optional time displacement. Block size is normalized from one pixel to the full frame, independent of source resolution.
...
18. **Signal Weave** alternates complete samples from A and B using moving line, field, band or checker patterns with resolution-independent normalized band sizing.
19. **Block Graft** evaluates one trigger per normalized spatial block and hold epoch, then replaces selected A samples with time-offset B samples.
20. **Channel Transplant** unpremultiplies A and the offset B sample, independently routes three RGB or Y/Cb/Cr components, reconstructs straight colour and re-premultiplies it against A's unchanged alpha. Clamp/Stretch preserve A's shape; Crop uses shared extents.

Shape-compatible nodes expose Amount compositing modes shared by proxy and mapped full render: Normal, Add, Screen, Multiply, Difference, a three-axis Displace field and quantized XOR Glitch. The mode and Amount are part of serialized creative state and the proxy cache key.
