# Сделано — ChronoForge 1.1.1

Релиз: **1.1.1**, 23 июля 2026 года.
...
- Searchable встроенная Help объясняет session workflow, Preview, A/B, compare, AA, FPS, queue/export, Amount/blend modes и термины обработки видео.
- Контекстный `?` в Inspector открывает страницу выбранного эффекта; общая справка находится в стандартном меню Help, а каталог автоматически проверяется против `EffectDefinition`.
...
- Каждый эффект поддерживает Amount от identity до полного результата; для shape-changing результата input сначала нормализуется к его форме.
- Blend modes: Normal, Add, Screen, Multiply, Difference, Displace и XOR Glitch.
- Shape-changing режимы предлагают те же blend modes после нормализации input по времени и пространству.
...
- Export Current Frame сохраняет выбранную позицию Viewer как full-resolution 8-bit RGBA PNG после всего effect stack, нормализует её по изменённой оси T и переиспользует full-render cache.
...
- Current Frame release-gate matrix проверяет 3 640 конфигураций: все дискретные режимы и их допустимые комбинации у каждого эффекта, скрытую ветку 3D Rotation, Output Prefilter и все семь Amount Blend, включая shape-changing эффекты после нормализации input к output. На movie и alpha-bearing PNG fixtures выполняются first/middle/last, A/B graph, premultiplied RGBA, cold/warm cache и 21 840 byte-for-byte сравнений с PNG Sequence.
- Исправлен premultiplied-alpha контракт для Cubic Space-Time Map, blend-режимов Time Feedback и общего Amount compositor; RGB теперь не превышает результирующую alpha в RAM и SSD-backed full render.
- Source/graph cache signatures сериализуются с canonical sorted keys и остаются стабильными при неизменном входе.
...
- [x] Full-resolution Export Current Frame с timeline mapping, alpha/parity, cache reuse, progress и cancel.
- [x] Встроенная searchable Help со страницами всех production effects и автоматической проверкой против registry.
