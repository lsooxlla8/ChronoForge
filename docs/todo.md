...
## 5. Drag-and-drop импорта

Позже добавить перетаскивание файлов и последовательностей из Finder прямо в окно ChronoForge:

- movie-файл в пустую сессию становится primary media A;
- дополнительные movie-файлы добавляются в Media pool;
- набор совместимых изображений распознаётся как одна image sequence, а не как россыпь отдельных файлов;
- drop target заранее показывает, будет файл импортирован как A, B или sequence;
- неподдерживаемые и неоднозначные наборы отклоняются с понятным сообщением, без частичного импорта.
